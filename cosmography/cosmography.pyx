    ...
    * Input: 
    ...
    def __cinit__(self, double H0, double omm, double omb=0.04,
        ...
        MODS = {'lcdm':0, 'wcdm':1, 'wwacdm': 2, 'edecdm':3, \
                'IDE01': 4, 'IDE02': 5, 'IDE03': 6}
        self.mod = MODS[MOD]
        if(self.mod == 5 or self.mod == 6):
...
#
cdef class lcdm_bk:
    cdef double h0, omm, omk

    def __cinit__(self, double h0, double omm, double omk):
        self.h0 = h0
        self.omm = omm
        self.omk = omk
        self.c = 2.99792458e5 #[km/s]
    ...
    cpdef double Hofz(self, double z):
        cdef double Ez2
        Ez2 = self.omm*(1+z)**3 +self.omk*(1+z)**2 +(1-self.omm -self.omk)
        return self.h0*sqrt(Ez2)
    ...
    cpdef double dmz(self, double z):
        cdef double dz, err
        cdef double fk, sk
        dz, err = integrate.quad(self.dtauda, 0, z)
        fk = self.c/self.h0
        sk = sqrt(fabs(self.omk))
        if(self.omk > 1e-8):
            return fk/sk * sinh(sk*dz)
        elif(self.omk < -1e-8):
            return fk/sk * sin(sk*dz)
        return fk * dz
