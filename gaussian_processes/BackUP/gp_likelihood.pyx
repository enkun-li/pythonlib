...
cdef class GaussianProcesses:
    ...
    cpdef double kernel(self,double x, double y):
        ...
        '''
        cdef double k
        k = self.sigf**2*np.exp(-(x-y)**2/2/self.lenf**2)
        return k
    ...
    cpdef double log_likelihood(self):
        ...
        chisq += self.logDet + self.n*np.log(2*np.pi)
        chisq = -0.5*chisq
        return chisq
...
cdef class rec_GaussianProcesses(GaussianProcesses):
    ...
    def rec_mu(self,x):
        ...
        if(n==1):
            vec = np.array([self.kernel(x,xi) for xi in self.X])
            iMY = np.matmul( self.IMcov, self.Y)
            mu = np.dot(vec, iMY)
        else:
            mu = np.zeros(n)
            for i in range(n):
                vec = np.array([self.kernel(x[i],xi) for xi in self.X])
                iMY = np.matmul( self.IMcov, self.Y )
                mu[i] = np.dot(vec, iMY)
        return mu
    ...
    def rec_dmu(self,x):
        ...
        try:
            ...
            n=1
        if(n==1):
            vec = np.array([self.__dkerneldx(x,xi) for xi in self.X])
            iMY = np.matmul( self.IMcov, self.Y)
            mu = np.dot(vec, iMY)
        else:
            mu = np.zeros(n)
            for i in range(n):
                vec = np.array([self.__dkerneldx(x[i],xi) for xi in self.X])
                iMY = np.matmul( self.IMcov, self.Y )
                mu[i] = np.dot(vec, iMY)
        return mu
    ...
    def rec_ddmu(self,x):
        ...
        try:
            ...
            n=1
        if(n==1):
            vec = np.array([self.__d2kerneldxdx(x,xi) for xi in self.X])
            iMY = np.matmul( self.IMcov, self.Y)
            mu = np.dot(vec, iMY)
        else:
            mu = np.zeros(n)
            for i in range(n):
                vec = np.array([self.__d2kerneldxdx(x[i],xi) for xi in self.X])
                iMY = np.matmul( self.IMcov, self.Y )
                mu[i] = np.dot(vec, iMY)
        return mu
    ...
    def rec_covarianve(self, x):
        ...
        if(n==1):
            v = self.kernel(x,x)
            vecl = np.array([self.kernel(x,xi) for xi in self.X])
            vecr = np.array([self.kernel(xi,x) for xi in self.X])
            iMY = np.matmul( vecl, self.IMcov )
            cov = v - np.dot(iMY, vecr)
        else:
            cov = np.zeros((n,n))
            v = np.zeros((n,n))
            for i in range(n):
                for j in range(n):
                    v[i,j] = self.kernel(x[i],x[j])
                    vecl = np.array([self.kernel(x[i],xi) for xi in self.X])
                    vecr = np.array([self.kernel(xi,x[j]) for xi in self.X])
                    iMY = np.matmul( vecl, self.IMcov )
                    cov[i,j] = v[i,j] -np.dot(iMY, vecr)
        return cov
    ...
    def rec_dfdfcovarianve(self, x):
        ...
        if(n==1):
            v = self.__d2kerneldxdy(x,x)
            vecl = np.array([self.__dkerneldx(x,xi) for xi in self.X])
            vecr = np.array([self.__dkerneldy(xi,x) for xi in self.X])
            iMY = np.matmul( vecl, self.IMcov )
            cov = v - np.dot(iMY, vecr)
        else:
            cov = np.zeros((n,n))
            ...
            for i in range(n):
                for j in range(n):
                    v[i,j] = self.__d2kerneldxdy(x[i],x[j])
                    vecl = np.array([self.__dkerneldx(x[i],xi) for xi in self.X])
                    vecr = np.array([self.__dkerneldy(xi,x[j]) for xi in self.X])
                    iMY = np.matmul( vecl, self.IMcov )
                    cov[i,j] = v[i,j] -np.dot(iMY, vecr)
        return cov
    ...
    def rec_ddfddfcovarianve(self, x):
        ...
        if(n==1):
            v = self.__d4kerneldx2dy2(x,x)
            vecl = np.array([self.__d2kerneldxdx(x,xi) for xi in self.X])
            vecr = np.array([self.__d2kerneldydy(xi,x) for xi in self.X])
            iMY = np.matmul( vecl, self.IMcov )
            cov = v - np.dot(iMY, vecr)
        else:
            cov = np.zeros((n,n))
            ...
            for i in range(n):
                for j in range(n):
                    v[i,j] = self.__d4kerneldx2dy2(x[i],x[j])
                    vecl = np.array([self.__d2kerneldxdx(x[i],xi) 
                        for xi in self.X])
                    vecr = np.array([self.__d2kerneldydy(xi,x[j]) 
                        for xi in self.X])
                    iMY = np.matmul( vecl, self.IMcov )
                    cov[i,j] = v[i,j] -np.dot(iMY, vecr)
        return cov   
    ...
    def rec_diff_covarianve(self, x, dx=1,dy=1):
        ...
        if(n==1):
            v = self.__diff_kernel(x,x, dx, dy)
            vecl = np.array([self.__diff_kernel(x,xi,dx,0) for xi in self.X])
            vecr = np.array([self.__diff_kernel(xi,x,0,dy) for xi in self.X])
            iMY = np.matmul( vecl, self.IMcov )
            cov = v - np.dot(iMY, vecr)
        else:
            cov = np.zeros((n,n))
            ...
            for i in range(n):
                for j in range(n):
                    v[i,j] = self.__diff_kernel(x[i],x[j],dx,dy)
                    vecl = np.array([self.__diff_kernel(x[i],xi,dx,0)
                        for xi in self.X])
                    vecr = np.array([self.__diff_kernel(xi,x[j],0,dy)
                        for xi in self.X])
                    iMY = np.matmul( vecl, self.IMcov )
                    cov[i,j] = v[i,j] -np.dot(iMY, vecr)
        return cov
    ...
    cpdef double rec_diff_covarianve_x_y(self, double x, double y, int dx=1, int dy=1):
        ...
        v = self.__diff_kernel(x,y,dx,dy)
        vecl = np.array([self.__diff_kernel(x,xi,dx,0) for xi in self.X])
        vecr = np.array([self.__diff_kernel(xi,y,0,dy) for xi in self.X])
        iMY = np.matmul( vecl, self.IMcov )
        cov = v - np.dot(iMY, vecr)
        return cov
