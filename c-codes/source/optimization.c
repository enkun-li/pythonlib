...
double aimfunc_SN(const gsl_vector *v, void *pars)
{
    ...
    setup_gapp(sigf, lenf);
    chisq = -2*loglikelihood();
    free_gapp();
    return chisq;
}


int multi_dim_mini(double(*func)(const gsl_vector *, void *), double sigf, double lenf)
{
    double par[] = {0.0};
    ...
    gsl_vector_set (x, 0, sigf);
    gsl_vector_set (x, 1, lenf);
...
int main(void)
{
    //multi_dim_mini(&aimfunc, 130.0, 2.0);
    multi_dim_mini(&aimfunc_SN, 1.6, 2.0);
    return 0;
}
